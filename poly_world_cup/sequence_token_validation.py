...
def recount_tokens(dataset_root,tokenizer_path,template_path,*,workers=7,progress=print):
    ...
    files = []
    tasks = ((str(root),name,metadata) for name,metadata in sorted(entries))
    ...
    return {"schema_version":1,"status":"passed","all_rows_recomputed":True,
        "method":"full_apply_chat_template_no_truncation","manifest_sha256":manifest_sha,
